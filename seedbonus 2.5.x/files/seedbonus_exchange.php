...
$r->data_seek(0); $p=$r->fetch_array()["points"];
$r->data_seek(0); $t=$r->fetch_array()["traffic"];
$uid=$CURUSER["uid"];
$r=do_sqlquery("SELECT seedbonus FROM {$TABLE_PREFIX}users WHERE id=$uid");
$r->data_seek(0); $u=$r->fetch_array()["seedbonus"];
if($u<$p) {
...
@$mysqli->query("UPDATE {$TABLE_PREFIX}users SET uploaded=uploaded+$t,seedbonus=seedbonus-$p WHERE id=$uid");
header("Location: index.php?page=modules&module=seedbonus");
}}
